  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    filterBtns.forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        let visible = 0;
        cards.forEach((card) => {
          const catList = (card.dataset.cats || '').split(',').filter(Boolean);
          const show = filter === 'all' || catList.includes(filter);
        ...
        if (visible === 0) {
          ...
          icon.textContent = '○';
          const msg  = document.createTextNode('No projects in this category yet.');
