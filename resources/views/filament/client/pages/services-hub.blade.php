<x-filament-panels::page>
    @php
        $serviceRows = [
            [
                ...
                'type' => 'Domain',
                'status' => ['label' => 'Active', 'class' => 'bg-[var(--i2-color-success-soft)] text-[var(--i2-color-success)]'],
                'due' => 'May 15, 2025',
            ...
            [
                'icon' => 'lock',
                'iconClass' => 'bg-[var(--i2-color-neutral-100)] text-[var(--i2-color-text-soft)]',
                'title' => 'SiteLock Security',
                'subtitle' => 'Website Security',
                'type' => 'Add-on',
                'status' => ['label' => 'Expiring Soon', 'class' => 'bg-[var(--i2-color-warning-soft)] text-[var(--i2-color-warning)]'],
                'due' => 'Jun 05, 2025',
    ...
    <div class="font-[var(--i2-font-sans)] text-[var(--i2-color-text)]">
        <main>
            <section aria-label="Service summary" class="mb-[22px] grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5">
                <x-client.stat-card label="Total Services" value="8" note="All services" icon-class="bg-[var(--i2-color-accent-soft)] text-[var(--i2-color-accent)]">
                    <x-slot:icon>
                ...
                <x-client.stat-card label="Active" value="6" note="75% of total" icon-class="bg-[var(--i2-color-success-soft)] text-[var(--i2-color-success)]">
                    <x-slot:icon>
                ...
                <x-client.stat-card label="Expiring Soon" value="2" note="25% of total" icon-class="bg-[var(--i2-color-info-soft)] text-[var(--i2-color-info)]">
                    <x-slot:icon>
                ...
                <x-client.stat-card label="Suspended" value="0" note="0% of total" icon-class="bg-[var(--i2-color-warning-soft)] text-[var(--i2-color-warning)]">
                    <x-slot:icon>
                ...
                <x-client.stat-card label="Cancelled" value="0" note="0% of total" icon-class="bg-[var(--i2-color-danger-soft)] text-[var(--i2-color-danger)]">
                    <x-slot:icon>
            ...
            <x-client.panel padding="none">
                <div class="flex flex-col items-start justify-between gap-4 px-[22px] pb-0 pt-[18px] xl:flex-row xl:items-center">
                    <div class="flex w-full items-center gap-7 overflow-x-auto pb-1" aria-label="Service filters">
                        <button type="button" class="relative whitespace-nowrap pb-[18px] pt-2 text-[var(--i2-color-accent)] after:absolute after:bottom-0 after:left-[-6px] after:right-[-6px] after:h-[3px] after:rounded-full after:bg-[var(--i2-color-accent)] after:content-['']">All Services</button>
                    ...
                    <div class="flex w-full flex-col gap-3 xl:w-auto xl:flex-row xl:items-center">
                        <button type="button" class="inline-flex min-h-9 items-center justify-center gap-2.5 rounded-[10px] border border-[var(--i2-color-border-soft)] bg-[var(--i2-color-surface)] px-[18px] text-[var(--i2-color-text-soft)]">
                        ...
                        <button type="button" class="inline-flex min-h-9 items-center justify-center gap-2.5 rounded-[10px] bg-[var(--i2-color-brand-ink)] px-[18px] text-white">
                ...
                <div class="px-[22px] pb-0 pt-5">
                    <div class="hidden items-center gap-[18px] border-t border-[var(--i2-color-border-soft)] px-[10px] py-4 text-[.94rem] text-[var(--i2-color-text-soft)] xl:grid xl:grid-cols-[minmax(250px,2fr)_120px_120px_150px_100px_110px_132px] 2xl:grid-cols-[minmax(280px,2fr)_130px_120px_160px_110px_120px_142px]">
                        <span>Service</span>
                        <span>Type</span>
                        <span>Status</span>
                    ...
                    @foreach ($serviceRows as $service)
                        <article class="grid gap-3 border-t border-[var(--i2-color-border-soft)] px-[10px] py-4 xl:grid-cols-[minmax(250px,2fr)_120px_120px_150px_100px_110px_132px] xl:items-center xl:gap-[18px] 2xl:grid-cols-[minmax(280px,2fr)_130px_120px_160px_110px_120px_142px]">
                            <div class="flex min-w-0 items-center gap-4">
                                <div class="{{ $service['iconClass'] }} grid size-10 shrink-0 place-items-center rounded-xl">
                                ...
                                <div class="min-w-0">
                                    <h2 class="mb-1.5 text-[0.99rem] font-semibold tracking-[-0.02em]">{{ $service['title'] }}</h2>
                                    <p class="text-[var(--i2-color-text-faint)]">{{ $service['subtitle'] }}</p>
                                </div>
                            </div>

                            <div class="text-[var(--i2-color-text-faint)]">{{ $service['type'] }}</div>

                            <div>
                                <span class="{{ $service['status']['class'] }} inline-flex min-h-7 items-center justify-center rounded-[8px] px-[10px] text-[0.82rem] font-semibold whitespace-nowrap">
                                    {{ $service['status']['label'] }}
                                </span>
                            </div>

                            <div>
                                <strong class="text-[0.95rem] font-medium text-[var(--i2-color-text-soft)]">{{ $service['due'] }}</strong>
                                <span class="mt-2 block font-semibold text-[var(--i2-color-accent)]">{{ $service['dueNote'] }}</span>
                            </div>

                            <div class="text-[0.95rem] font-medium text-[var(--i2-color-text-soft)]">{{ $service['amount'] }}</div>
                            <div class="text-[var(--i2-color-text-faint)]">{{ $service['cycle'] }}</div>

                            <div class="flex items-center gap-2.5">
                                <button type="button" class="inline-flex min-h-9 min-w-20 items-center justify-center rounded-[10px] border border-[var(--i2-color-border-soft)] bg-[var(--i2-color-surface)] px-[14px] text-[var(--i2-color-brand-ink)]">Manage</button>
                                <button type="button" class="inline-flex h-9 w-9 items-center justify-center rounded-[10px] border border-[var(--i2-color-border-soft)] bg-[var(--i2-color-surface)] text-[var(--i2-color-text-soft)]" aria-label="More actions for {{ $service['title'] }}">
                ...
                <div class="flex flex-col items-start justify-between gap-4 px-[22px] pb-[14px] pt-[18px] xl:flex-row xl:items-center">
                    <p class="m-0 text-[var(--i2-color-text-faint)]">Showing 1 to 8 of 8 services</p>
                    <div class="flex items-center gap-2.5">
                        <button type="button" class="inline-flex min-w-[34px] items-center justify-center rounded-[10px] border border-[var(--i2-color-border-soft)] bg-[var(--i2-color-surface)] px-3 text-[var(--i2-color-text-soft)]" aria-label="Previous page">
                        ...
                        <button type="button" class="inline-flex min-w-[34px] items-center justify-center rounded-[10px] bg-[var(--i2-color-brand-ink)] px-3 text-white">1</button>
                        <button type="button" class="inline-flex min-w-[34px] items-center justify-center rounded-[10px] border border-[var(--i2-color-border-soft)] bg-[var(--i2-color-surface)] px-3 text-[var(--i2-color-text-soft)]" aria-label="Next page">
