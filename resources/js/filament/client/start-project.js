import '../../../css/filament/client/start-project.css';

    const onboardingRoot = document.querySelector('[data-onboarding-submit-url]') || document.body;
    const visibleSteps = [2, 3, 4, 5];
    ...
    const fallbackUrl = onboardingRoot.dataset.onboardingFallbackUrl || '/';
    const submitUrl = onboardingRoot.dataset.onboardingSubmitUrl || window.location.pathname;
    ...
    function resolveServices() {
        const raw = onboardingRoot?.dataset?.onboardingCatalog;
    ...
    function resolvePreset() {
        const raw = onboardingRoot?.dataset?.onboardingPreset;
    ...
    function updateStepIndicator() {
        ...
        if (label) label.textContent = `Step ${currentIndex + 1} of ${totalSteps}`;

        visibleSteps.forEach((stepId, index) => {
            const item = document.querySelector(`.si-item[data-step="${stepId}"]`);
            const num = document.getElementById('si-num-' + stepId);
            if (!item || !num) return;
